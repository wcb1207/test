...
public class MuPDFCore {
	...
	private native int controlSepOnPageInternal(int page, int sep,
			boolean disable);
	...
	private void gotoPage(int page) {
		Log.i("pageSize", "page init:" + page + "|numPages:" + numPages);
		if (page > numPages - 1)
			page = numPages - 1;
		else if (page < 0)
			page = 0;
	...
	public synchronized PointF getPageSize(int page) {
		gotoPage(page);
		Log.i("pageSize", "getPageSize:" + pageWidth + "|" + pageHeight);
	...
	public synchronized void drawPage(Bitmap bm, int page, int pageW,
		...
		// add by runbao
		Log.i("file",
				this.pageHeight + "||pw:" + this.pageWidth
						+ Integer.toString(pageW) + "|h:"
						+ Integer.toString(pageH) + "|bm w:"
						+ Integer.toString(bm.getWidth()) + "|h:"
						+ Integer.toString(bm.getHeight()));
		Log.i("file1",
				Integer.toString(patchX) + "|h:" + Integer.toString(patchY)
						+ "|bm w:" + Integer.toString(patchW) + "|h:"
						+ Integer.toString(patchH));
		//
		...
		// bmΪ ��Ļ��ʾ��ͼƬ
		/*
		 * Log.e("img", "����ͼƬ"); File f = new File("/storage/emulated/0/.fyh/",
		 * "1.png"); if (f.exists()) { f.delete(); } try { FileOutputStream out
		 * = new FileOutputStream(f); bm.compress(Bitmap.CompressFormat.PNG, 90,
		 * out); out.flush(); out.close(); Log.i("img", "�Ѿ�����"); } catch
	...
	public synchronized void updatePage(Bitmap bm, int page, int pageW,
		...
		Log.i("file",
				"updatePageInternal" + this.pageHeight + "||pw:"
						+ this.pageWidth + Integer.toString(pageW) + "|h:"
						+ Integer.toString(pageH) + "|bm w:"
						+ Integer.toString(bm.getWidth()) + "|h:"
						+ Integer.toString(bm.getHeight()));
		Log.i("file1",
				"updatePageInternal" + Integer.toString(patchX) + "|h:"
						+ Integer.toString(patchY) + "|bm w:"
						+ Integer.toString(patchW) + "|h:"
						+ Integer.toString(patchH));
		...
		/*
		 * ͼƬ���治�� Log.e("img", "����ͼƬ"); File f = new
		 * File("/storage/emulated/0/.fyh/", "11.png"); if (f.exists()) {
		 * f.delete(); } try { FileOutputStream out = new FileOutputStream(f);
		 * bm.compress(Bitmap.CompressFormat.PNG, 90, out); out.flush();
		 * out.close(); Log.i("img", "�Ѿ�����"); } catch (FileNotFoundException e)
